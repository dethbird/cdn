...
export async function createMediaRecord(data) {
  ...
  const [result] = await pool.query(
    ...
    [
      ...
      data.title || data.originalFilename || null,
      data.caption || null,
      data.altText || null
    ]
  );
...
/**
 * Update media record fields (e.g. bytes, mime_type after conversion)
 * @param {number} id - Media ID
 * @param {Object} fields - Fields to update
 * @param {number} [fields.bytes] - Updated file size
 * @param {string} [fields.mimeType] - Updated MIME type
 * @returns {Promise<void>}
 */
export async function updateMediaRecord(id, fields) {
  const updates = [];
  const values = [];
  if (fields.bytes !== undefined) {
    updates.push('bytes = ?');
    values.push(fields.bytes);
  }
  if (fields.mimeType !== undefined) {
    updates.push('mime_type = ?');
    values.push(fields.mimeType);
  }
  if (updates.length > 0) {
    values.push(id);
    await pool.query(
      `UPDATE media SET ${updates.join(', ')}, updated_at = NOW(3) WHERE id = ?`,
      values
    );
  }
}
